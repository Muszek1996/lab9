...
#pragma omp task default(none) firstprivate(A,p_task, k_task) private(max_to_find) shared(a_max) 		//task
                {
                        max_to_find= A[p_task];
                        while(p_task<=k_task) {

                            if(max_to_find < A[p_task]) max_to_find = A[p_task];
                            p_task++;
                        }
                    #pragma omp critical
                    if(max_to_find> a_max) a_max=max_to_find;
                }
...
double bin_search_max_task(                                                                 //task
  double* A,
  int p,
  int r,
  int level
...
#pragma omp task default(none) firstprivate(A,p,r,q1,level) shared(max1)
	    {
	    	max1=bin_search_max_task(A,p,q1,level);
	    }

#pragma omp task default(none) firstprivate(A,p,r,q1,level) shared(max2)
	    {

			max2=bin_search_max_task(A,q1+1,r,level);
	    }
...
#pragma omp task final( level > max_level) default(none) firstprivate(A,p,r,q1,level) shared(max1)
	    {
	    	if(omp_in_final())
	    	{
	     		max1=search_max(A,p,q1);
	    	}
	    	else max1=bin_search_max_task(A,p,q1,level);
	    }

#pragma omp task final( level > max_level ) default(none) firstprivate(A,p,r,q1,level) shared(max2)
	    {
	    	if(omp_in_final())
	    	{
	     		max2=search_max(A,q1+1,r);
	    	}
	     	else max2=bin_search_max_task(A,q1+1,r,level);
	    }
