...
int main() {
    ...
    static int fthreadprivate;
#pragma omp threadprivate(fthreadprivate)
...
//    omp_set_dynamic(0);
    ...
    printf("przed wejsciem do obszaru rownoleglego aktualna liczba watkow %d, moj ID %d\n",
...
#pragma omp parallel default(none) shared(a_shared, e_atomic) private(b_private) firstprivate(c_firstprivate ) num_threads(7)
    {
        fthreadprivate = omp_get_thread_num();
        int i;
        int d_local_private;
